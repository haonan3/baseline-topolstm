...
def mean_precision_at_k(relevance_scores, K):
    mean_precision_at_k = np.mean([precision_at_k(r, K) for r in relevance_scores]).astype(np.float32)
...
def recall_at_k(relevance_score, K, M):
    ...
    if relevance_score.size != K:
        raise ValueError('Relevance score length < K')
    return np.sum(relevance_score)/min(K,M)
...
def get_relevance_scores(topk_filter, decoder_targets):
    ...
    for i in range(0, topk_filter.shape[0]):
        z = np.isin(topk_filter[i], decoder_targets[i]) # decoder_targets may have start_token but topk_filter doesn't.
        output.append(z)
