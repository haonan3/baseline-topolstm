...
def lstm_layer(tparams, state_below, options, seq_masks=None, topo_masks=None):
    ...
    rval, updates = theano.scan(_step,
                                sequences=[tensor.arange(n_timesteps), seq_masks, topo_masks, state_below],
                                outputs_info=[tensor.alloc(numpy_floatX(0.), n_timesteps, n_samples, dim_proj),
                                              tensor.alloc(numpy_floatX(0.), n_timesteps, n_samples, dim_proj)],
                                name='lstm_layers',
                                n_steps=n_timesteps)
    return rval[0][-1]
...
def build_model(tparams, options):
    ...
    # embedding lookup.
    # a = tparams['Wemb']
    # tparams['Wemb']: (n_words, dim_proj)
    embs = tparams['Wemb'][seqs.flatten()].reshape([n_timesteps,n_samples,options['dim_proj']])
    ...
    # h_mean is what we want to save
